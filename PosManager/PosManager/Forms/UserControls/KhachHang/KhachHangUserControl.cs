...
namespace PosManager.Forms.UserControls.KhachHang
{
    public partial class KhachHangUserControl : UserControl
    {
        ...
        public KhachHangUserControl(fHome home)
        {
            _fHome = home;
            InitializeComponent();
        ...
        private async void loadAccount(int pageIndex = 1, int pageSize = 1, string? searchString = "")
        {
            try
            {
                ...
                var accounts = await _KhachHangController.Search(GlobalModel.AccsessToken, pageIndex.ToString(), pageSize.ToString(), searchString);
                ...
                if (accounts != null && accounts.StatusCode == 200 && accounts.Data.Result.Count > 0)
                {
                    ...
                    dtgvAccount.Invoke((MethodInvoker)delegate
                    {
                        ...
                        foreach (var a in accounts.Data.Result)
                        {
                            ...
                            var index=  dtgvAccount.Rows.Add(i, a.TenKhachHang, a.SoDienThoai, a.DiaChi, a.NgaySinh, a.GioiTinh, capBac, "Chi Tiết", Properties.Resources.delete15, a.Id);
                            switch (a.CapBac)
                            {
                                case 0:
                                    {
                                        dtgvAccount.Rows[index].DefaultCellStyle.BackColor = Color.White;

                                        break;
                                    }
                                case 1:
                                    {
                                        dtgvAccount.Rows[index].DefaultCellStyle.BackColor = Color.LightGoldenrodYellow;
                                ...
                                case 2:
                                    {
                                        ...
                                        dtgvAccount.Rows[index].DefaultCellStyle.BackColor = Color.PaleTurquoise;
                                        break;
                                    }
                            }
        ...
        private async void dtgvAccount_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //            {
            //                SplashScreenManager.ShowForm(_fHome, typeof(WaitForm1), true, true, false);
            //                var accounts = await _KhachHangController.Delete(GlobalModel.AccsessToken, id);
            //                SplashScreenManager.CloseForm(false);
            //                if (accounts != null)
            //                {
        ...
        private async void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (listDelete.Count > 0)
            {
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    ...
                    {
                        var accounts = await _KhachHangController.Delete(GlobalModel.AccsessToken, item);
                    }
                    loadAccount(currentPage, pageSize, txtSearch.Text.Trim());
                }
            }

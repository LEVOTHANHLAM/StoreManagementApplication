...
namespace PosManager.Forms.UserControls.SanPham
{
    public partial class NhaCungCapUserControl : UserControl
    {
        ...
        public NhaCungCapUserControl(fHome home)
        {
            _fHome = home;
            InitializeComponent();
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                fThemNCC them = new fThemNCC(null);
                them.ShowDialog();
        ...
        private void ChiNhanhUserControl_Load(object sender, EventArgs e)
        {
            ...
            {
                permissionModel = GlobalModel.UserInfo.Permissions.FirstOrDefault(x => x.FunctionName == "NhaCungCapUserControl");
        ...
        private async void loadAccount(int pageIndex = 1, int pageSize = 1, string? searchString = "")
        {
            try
            {
                ...
                var accounts = await _suppliersController.Search(GlobalModel.AccsessToken, pageIndex.ToString(), pageSize.ToString(), searchString);
                ...
                if (accounts != null && accounts.StatusCode == 200 && accounts.Data.Result.Count > 0)
                {
                    ...
                    dtgvAccount.Invoke((MethodInvoker)delegate
                    {
                        ...
                        {
                            dtgvAccount.Rows.Add(i, a.MaNhaCungCap, a.TenNhaCungCap, a.DiaChi, a.SoDienThoai, Properties.Resources.Edit, Properties.Resources.delete15, a.Id);
                            i++;
                        }
                        row = i;
                    });
            ...
            {
                SplashScreenManager.CloseForm(false);
                Log.Error($"{nameof(NhaCungCapUserControl)}, params; {nameof(loadAccount)}, Error; {ex.Message}, Exception; {ex}");
        ...
        private void DisplayDataOnCurrentPage()
        {
            ...
            {
                Log.Error($"{nameof(NhaCungCapUserControl)}, params; {nameof(DisplayDataOnCurrentPage)}, Error; {ex.Message}, Exception; {ex}");
            }
        }
        private void dtgvAccount_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //    catch (Exception ex)
            //    {
            //        Log.Error($"{nameof(NhaCungCapUserControl)}, params; {nameof(dtgvAccount_CellClick)}, Error; {ex.Message}, Exception; {ex}");
            //        MessageCommon.ShowMessageBox(ex.Message, 4);
            //    }
            //}
        }
        ...
        private async void dtgvAccount_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex == dtgvAccount.Columns["cEdit"].Index)
            {
                if (permissionModel != null && permissionModel.HasUpdate)
                {
                    ...
                    fThemNCC themNhaCungCap = new fThemNCC(id);
                    SplashScreenManager.CloseForm(false);
            ...
            if (e.RowIndex >= 0 && e.ColumnIndex == dtgvAccount.Columns["cDelete"].Index)
            {
                if (permissionModel != null && permissionModel.HasUpdate)
                {
                    try
                    {
                        ...
                        {
                            var accounts = await _suppliersController.Delete(GlobalModel.AccsessToken, id);
                            SplashScreenManager.CloseForm(false);
                            if (accounts != null)
                            {
        ...
        private void dtgvAccount_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //    catch (Exception ex)
            //    {
            //        Log.Error($"{nameof(NhaCungCapUserControl)}, params; {nameof(dtgvAccount_CellDoubleClick)}, Error; {ex.Message}, Exception; {ex}");
            //        MessageCommon.ShowMessageBox(ex.Message, 4);
            //    }
            //}
        }
        ...
        private List<string> GetListSelect()
        {
            ...
            //catch (Exception ex)
            //{
            //    Log.Error($"{nameof(NhaCungCapUserControl)}, params; {nameof(GetListSelect)}, Error; {ex.Message}, Exception; {ex}");
            //    MessageCommon.ShowMessageBox(ex.Message, 4);
            //}
            return list;
        }
        ...
        private async void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (listDelete.Count > 0)
            {
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    ...
                    {
                        var accounts = await _suppliersController.Delete(GlobalModel.AccsessToken, item);
                    }
                    loadAccount(currentPage, pageSize, txtSearch.Text.Trim());
                }
            }

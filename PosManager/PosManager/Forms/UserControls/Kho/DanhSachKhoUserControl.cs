...
namespace PosManager.Forms.UserControls.Kho
{
    public partial class DanhSachKhoUserControl : UserControl
    {
        ...
        public DanhSachKhoUserControl(fHome home)
        {
            _fHome = home;
            InitializeComponent();
                ...
                if (accounts != null && accounts.StatusCode == 200 && accounts.Data.Result.Count > 0)
                {
                    ...
                    dtgvAccount.Invoke((MethodInvoker)delegate
                    {
                        ...
                        foreach (var a in accounts.Data.Result)
                        {
                            ...
                            dtgvAccount.Rows.Add(i, a.MaKho, a.TenKho, Properties.Resources.Edit, Properties.Resources.delete15, a.Id);
                            i++;
                        }
                        row = i;
                    });
            ...
            if (e.RowIndex >= 0 && e.ColumnIndex == dtgvAccount.Columns["cDelete"].Index)
            {
                if (permissionModel != null && permissionModel.HasUpdate)
                {
                    try
                    {
                        ...
                        {
                            var accounts = await _stocksController.Delete(GlobalModel.AccsessToken, id);
                            SplashScreenManager.CloseForm(false);
                            if (accounts != null)
                            {
